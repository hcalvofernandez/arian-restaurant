...
class AccountAccount(models.Model):
    ...
    expense_element_detailed = fields.Boolean(
        string='Expense element detailed?',
        compute='_compute_expense_element_detailed',
        readonly=False
    )
...
class AccountGroup(models.Model):
    ...
    def _constraint_prefix_overlap(self):
        # Odoo 17 CE ya maneja la validación de solapamiento de prefijos
        pass
    ...
    def _compute_parent(self):
        ...
        for record in self.filtered('group_template_parent_id'):
            parent = record_parents.filtered(
                lambda it: it.group_template_id == record.group_template_parent_id and it.company_id == self.env.company
            )
            if parent:
